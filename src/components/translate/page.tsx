  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <select
            ...
            <optgroup label="Public Conlangs">
              {publicConlangs.map(lang => (
                <option key={lang.id} value={lang.id}>{lang.name} (by {lang.userName || lang.userEmail})</option>
              ))}
            </optgroup>
          </select>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <textarea
            ...
            className="w-full h-40 p-4 border-2 border-indigo-200 rounded-lg focus:border-indigo-500 focus:outline-none resize-none"
      ...
      <button
        ...
        className="w-full px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {isTranslating ? (
          <>
